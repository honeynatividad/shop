		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				if(isset($this->session->data['certno'])){
					...
					$vendor_id = $this->session->data['vendor_id'];
					$quer = "SELECT product_price FROM vendor_checker WHERE vendor_id='".$vendor_id."' AND product_id='" . (int)$cart['product_id'] . "'";
					$vendor_query = $this->db->query($quer);
					
					$price = $vendor_query->row['product_price'];
				}
				// Product Discounts
				$discount_quantity = 0;
				...
				$product_data[] = array(
					...
					'recurring'       => $recurring,
					'product_type'		=> $product_query->row['product_type']	//edited by HCN
				);
			} else {
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		...
		if (!$query->row['total']) {
			if(isset($this->session->data['vendor_id'])){
				...
				$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', vendor_id = '" . $vendor_id. "', certno = '" . $certno . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
			}else{
