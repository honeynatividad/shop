...
class ModelPaymentAurumpay extends Model {
    
    public function getMethod($address) {
        
        $this->load->language('payment/aurumpay');
		
	if ($this->config->get('aurumpay_status')) {
            ...
            if (!$this->config->get('aurumpay_geo_zone_id')) {
                $status = TRUE;
	...
	if ($status) {  
		...
		'terms'      => '',
		'sort_order' => $this->config->get('aurumpay_sort_order')
            );
    	}
