...
class ControllerCheckoutGuest extends Controller {
	public function index() {
		...
		//edited by HCN
		if (isset($this->session->data['guest']['first_time'])) {
			$data['first_time'] = $this->session->data['guest']['first_time'];
		} else {
			$data['first_time'] = '';
		}
		...
		if(isset($this->session->data['certno'])){
			$data['certno'] = $this->session->data['certno'];
		}else{
			$data['certno'] = "";
		}
	...
	public function save() {
		...
		if (!$json) {
			...
			$this->load->model('localisation/country');
			//MANUAL of setting COUNTRY ID
			//country_id=168
			$country_info = $this->model_localisation_country->getCountry(168);
			...
			//edited by HCN
			if ($this->request->post['first_time'] == '') {
				$json['error']['first_time'] = $this->language->get('error_first_time');
			}*/
		...
		if (!$json) {
			...
			$this->session->data['guest']['first_time'] = $this->request->post['first_time'];*/
			...
			$this->session->data['payment_address']['country_id'] = '168';
			$this->session->data['payment_address']['zone_id']	=	'4225';
			...
			$zone_info = $this->model_localisation_zone->getZone('4225');
			...
			//edited by HCN
			if (!empty($this->request->post['first_time'])) {
				$this->session->data['guest']['first_time'] = $this->request->post['first_time'];
			} else {
				$this->session->data['guest']['first_time'] = false;
			}
