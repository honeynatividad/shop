...
class ControllerPaymentIpay88 extends Controller {
	...
	// Assign values for form post
		...
		$data['ResponseURL'] = 'https://shop.philcare.com.ph/payment-gateway/callback.php';
    ...
    public function backendcallback(){
		...
		if ($_SESSION['Status']=="1" && $check_sign==$expected_sign){
			...
			$x=1;
			foreach ($this->cart->getProducts() as $product) {
				/*if($product['model']=="ERVPA40" || $product['model']=="ERVPK40" || $product['model']=="ERVPA60" || $product['model']=="ERVPK60" || $product['model']=="ERVPA80" || $product['model']=="ERVPK80" || $product['model']=="DACA" || $product['model'] == "ERS" || $product['model'] == "MDC65" || $product['model'] == "MDCK" || $product['model'] == "MDCA" || $product['model'] == "MDCK" || 
					$product['model'] == 'PEARLNP60A1' || $product['model'] == 'PEARLP60A1' || $product['model'] == 'PEARLNP60A2' || $product['model'] == 'PEARLP60A2' || $product['model'] == 'PEARLNP60K' ||
					$product['model'] == 'PEARLP60K' || $product['model'] == 'EMERALDNP80A1' || $product['model'] == 'EMERALDP80A1' || $product['model'] == 'EMERALDNP80A2' || $product['model'] == 'EMERALDP80A1' || $product['model'] == 'EMERALDNP80K' || $product['model'] == 'EMERALDP80AK' || $product['model'] == 'DIAMONDNP100A1' || $product['model'] == 'DIAMONDP100A1' || $product['model'] == 'DIAMONDNP100A2' || $product['model'] == 'DIAMONDP100A2' || $product['model'] == 'DIAMONDNP100K' || $product['model'] == 'DIAMONDP100K' || $product['model'] == 'PEARLNPW60A1' || $product['model'] == 'PEARLPW60A1' || $product['model'] == 'PEARLNPW60A2' || $product['model'] == 'PEARLPW60A2' || $product['model'] == 'PEARLNPW60K' || $product['model'] == 'PEARLPW60AK' || $product['model'] == 'EMERALDNPW80A1' || $product['model'] == 'EMERALDPW80A1' || $product['model'] == 'EMERALDNPW80A2' || $product['model'] == 'EMERALDPW80A2' || $product['model'] == 'EMERALDNPW80K' || $product['model'] == 'EMERALDPW80K' || $product['model'] == 'DIAMONDNPW100A1' || $product['model'] == 'DIAMONDPW100A1' || $product['model'] == 'DIAMONDNPW100A2' || $product['model'] == 'DIAMONDPW100A2' || $product['model'] == 'DIAMONDNPW100K' || $product['model'] == 'DIAMONDPW100K'){*/

					$newDate = date("m/d/Y", strtotime($order_info['date_modified']));

					$url = "https://apps.philcare.com.ph/IPhilCareECard/Ecommerce.svc/GetECardLists/?VendorID=".$order_info['vendor_id']."&InvoiceNo=".$order_info['order_id']."-".$x."&InvoiceDate=".$newDate."&FirstName=".urlencode($order_info['payment_firstname'])."&LastName=".urlencode($order_info['payment_lastname'])."&MiddleName=&Email=".urlencode($order_info['email'])."&TelNo=".urlencode($order_info['telephone'])."&CompanyName=&Adr1=".urlencode($order_info['payment_address_1'])."&Adr2=".urlencode($order_info['payment_address_2'])."&Adr3=&City=".urlencode($order_info['payment_city'])."&Country=".urlencode($order_info['payment_country'])."&PostalCode=".urlencode($order_info['payment_postcode'])."&Shipping=&Total=".$order_info['total']."&OrderStatus=S&PaymentMethod=".urlencode($order_info['payment_code'])."&ProductName=".$product['model']."&ProductType=".$product['product_type']."&ProductModel=".$product['model']."&Quantity=".$product['quantity']."&Remarks=&MID=&BuyerCertNo=".$order_info['certno'];
            ...
            $data['continue'] = $this->url->link('checkout/success', '', 'SSL');
			$data['url_api']	= $url;
			if($vendor_id != "38412629-7A2D-424D-872E-68F16214826F"){//check if not a philcare e commerce vendor
				$data['continue'] = "https://philcare.com.ph/gateway";
				unset($this->session->data['certno']);
				$this->cart->clear();
			}
